...
public:
	...
	static void ConstructParams(const SemanticApi::IActualParameters* curr_op, std::vector<TStackValue> &method_call_formal_params, TStatementRunContext& run_context);
	static void DestroyParams(std::vector<TStackValue> &method_call_formal_params, TStatementRunContext& run_context);
