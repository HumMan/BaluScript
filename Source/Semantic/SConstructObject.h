...
public:
	...
	void Build(TTokenPos* source, std::vector<TExpressionResult>& exp_results, std::vector<TSOperation*>& params, std::vector<TFormalParameter>& params_formals, TGlobalBuildContext build_context);
	void Build(TTokenPos* source, std::vector<std::unique_ptr<TExpression>>& params, TGlobalBuildContext build_context);
